...
namespace AndyB.Comms.OldSerial
{
	...
	internal partial class Win32Comm
	{
		...
		unsafe internal IAsyncResult BeginWrite(byte[] buf, int offset, int size, AsyncCallback? callback, object? state)
		{
			...
			// Create a managed overlapped class and pack it and save in the async result
			var overlapped = 
				new Overlapped(0, 0, result.AsyncWaitHandle.SafeWaitHandle.DangerousGetHandle(), result);
				//new Overlapped(0, 0, IntPtr.Zero, result);
			...
			fixed (byte* p = buf)
            {
				...
				if (!WriteFile(_handle, p + offset, (uint)size, IntPtr.Zero, pOverlapped))
				{
					var status = Marshal.GetLastWin32Error();
					if (status != Win32Comm.ERROR_IO_PENDING)
						throw new SerialException();
		...
		unsafe public void EndWrite (IAsyncResult iar)
        {
			...
			_logger.Debug("EndWrite completed");
        }
		...
		/// <param name="offset">The starting offset in <paramref name="buf"/> to start writing from.</param>
		/// <param name="count">Number to bytes to send.</param>
		internal bool Write(byte[] buf, int offset, int count)
		{
#if true
			var result = BeginWrite(buf, offset, count, null, null);
		...
		unsafe private void AsyncFSCallback(uint errorCode, uint numBytes, NativeOverlapped* pOverlapped)
        {
			...
			// That's because we didn't set an event in the overlapped structure
			_logger.Debug($"Wait event waiting");
			if (asyncResult.AsyncWaitHandle != null && !asyncResult.AsyncWaitHandle.SafeWaitHandle.IsInvalid)
				asyncResult.AsyncWaitHandle.WaitOne();
				//asyncResult.WaitEvent.WaitOne();
			_logger.Debug($"Wait event signalled");
			...
			// Tell end result that we're done.
			asyncResult.WaitEvent.Set();
		...
		internal void SetQueues (uint inQueue, uint outQueue)
        {
			if (!SetupComm(_handle, inQueue, outQueue))
				throw new SerialException();
        }
