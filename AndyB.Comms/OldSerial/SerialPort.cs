...
namespace AndyB.Comms.OldSerial
{
    ...
    public class SerialPort : Component //IDisposable
    {
        ...
        private uint _writeMultiplerTimeout;
        private uint _readTimeout;
        private uint _readIntervalTimeout;
        private uint _readMultiplyTimeout;
        private bool _useOverlapped = true;
...
//        [DefaultValue(_defaultStopBits)]
        ...
        /// <summary>
        /// Gets/sets the control of the CTS handshake.
        /// </summary>
        /// <remarks><para>When set, transmission will be halted
        /// unless the CTS pin is active.
        /// </para></remarks>
        ...
        /// <summary>
        /// Gets/sets if the receiver is sensitive to the DSR pin.
        /// </summary>
        /// <remarks><para>If <c>true</c> the driver ignores any received
        /// bytes unless DSR is asserted.
        /// </para></remarks>
        ...
        /// <summary>
        /// Gets/sets if the transmitter continues when the receive buffer is full.
        /// </summary>
        /// <remarks><para>If <c>true</c>, transmission continues when the input buffer has reached the XOFF limit
        /// of becoming full and XOFF has been transmitted; otherwise the transmission does not continue when the input buffer has fallen below the
        /// XOFF limit and XON has been transmitted.
        /// </para></remarks>
        ...
        public bool ParityReplace
        {
            get => _settings.ParityReplace;
            set
            {
                if (IsOpen)
                {
                    _dcb.ParityReplace = value;
                    _dcb.Update();
                }
                _settings.ParityReplace = value;
            }
        }

        public bool DiscardNull
        {
            get => _settings.DiscardNull;
            set
            {
                if (IsOpen)
                {
                    _dcb.DiscardNull = value;
                    _dcb.Update();
                }
                _settings.DiscardNull = value;
            }
        }
        public bool AbortOnError
        {
            get => _settings.AbortOnError;
            set
            {
                if (IsOpen)
                {
                    _dcb.AbortOnError = value;
                    _dcb.Update();
                }
                _settings.AbortOnError = value;
            }
        }
        ...
        /// <summary>
        /// Gets/sets the received interval timeout.
        /// </summary>
        public uint RxIntervalTimeout
        {
            get => _readIntervalTimeout;
            set
            {
                if (IsOpen)
                {
                    _timeout.ReadInterval = value;
                    _timeout.Set();
                }
                _readIntervalTimeout = value;
            }
        }
        ...
        {
            get => _readMultiplyTimeout;
            set
            {
                if (IsOpen)
                {
                    _timeout.ReadMultiplier = value;
                    _timeout.Set();
                }
                _readMultiplyTimeout = value;
            }
        }
        ...
        {
            get => _writeMultiplerTimeout;
            set
            {
                if (IsOpen)
                {
                    _timeout.WriteMultiplier = value;
                    _timeout.Set();
                }
                _writeMultiplerTimeout = value;
            }
        }

        /// <summary>
        /// Gets/sets the transmit timeout.
        /// </summary>
...
#region Read / Write
        ...
        public void Write (byte [] bytes, int offset, int count)
        {
            ...
            _comm.Write(bytes, offset, count);
...
#region Static Methods
        ...
        private class EventThreadRunner : IDisposable
        {
            ...
            internal EventThreadRunner(SerialPort port)
            {
                ...
                if (!port._useOverlapped)
                {
                    var thread = new Thread(new ThreadStart(Background));
                    thread.Start();
                }
            }
                ...
                while (true)
                {
                    try
                    {
                        ...
                        if (evt.HasFlag(WinEvents.RxChar))
                        {
                            ...
                            charCount++;
                            _logger.Trace($"No of RX chars: {charCount}");
                        }
                ...
                while (true)
                {
                    try
                    {
                        ...
                        if (evt.HasFlag(WinEvents.RxChar))
                        {
                                // Character received
                                _logger.Trace("Event: RXCHAR");
                                _logger.Trace($"Before - no of RX chars: {charCount}");
                                var buf = new byte[charCount];
                                charCount++;
                                _port._comm.Read(buf, 0, out uint nRead);
                                Array.Resize(ref buf, (int)nRead);
                                charCount -= nRead;
                                _logger.Trace($"After - no of RX chars: {charCount}");

                                _port.OnDataReceived(buf);
                        }
