...
namespace AndyB.Comms.OldSerial
{
	...
	public class Win32Dcb : IDisposable
    {
		...
		public void Initialise(SerialSettings settings)
        {
			...
			_dcb.XonChar = (byte)0x11; /* DC1 CTRL-Q (resume) */
			_dcb.XoffChar = (byte)0x13; /* DC3 CTRL-S (pause) */
			_dcb.ErrorChar = (byte)'?';
			_dcb.EvtChar = (byte)'?';
			_dcb.EofChar = (byte)'?';
		...
		private void SetDcbFlag (int whichFlag, bool setting) =>
		...
		internal bool RxDsrSense
        {
			...
			set => SetDcbFlag(FDSRSENSITIVITY, value);
        }

		internal bool TxContinue
        {
			get => GetDcbFlag(FTXCONTINUEONXOFF) != 0;
			set => SetDcbFlag(FTXCONTINUEONXOFF, value);
        }

		internal bool TxFlowXoff
        {
			get => GetDcbFlag(FOUTX) != 0;
			set => SetDcbFlag(FOUTX, value);
        }

		internal bool RxFlowXoff
        {
			get => GetDcbFlag(FINX) != 0;
			set => SetDcbFlag(FINX, value);
        }

		internal bool ParityReplace
        {
			get => GetDcbFlag(FPARITY) != 0;
			set => SetDcbFlag(FPARITY, value);
        }

		internal bool DiscardNull
        {
			get => GetDcbFlag(FNULL) != 0;
			set => SetDcbFlag(FNULL, value);
        }

		internal bool AbortOnError
        {
			get => GetDcbFlag(FABORTONOERROR) != 0;
			set => SetDcbFlag(FABORTONOERROR, value);
        }
