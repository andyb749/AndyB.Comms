...
//		/// </summary>
		...
		/// </summary>
		protected internal Win32Status(IntPtr handle)
		{
			_handle = handle;
			this.ready = 0;
			this.cs = new COMMSTAT();
			this._state = new COMMHOLD();
			this._error = new COMMERRS();
			return;
		}
		...
		{
			if (ClearCommError(_handle, out this._error.status, out this.cs) == false)
			{
//				this.SetFault("ClearCommError(Clear)");
//				return false;
				throw new CommsException();
			}
			this._error.overflowError = ((this._error.status & CE_RXOVER) != 0);
			this._error.overrunError = ((this._error.status & CE_OVERRUN) != 0);
			this._error.parityError = ((this._error.status & CE_RXPARITY) != 0);
			this._error.framingError = ((this._error.status & CE_FRAME) != 0);
			this._error.breakCondition = ((this._error.status & CE_BREAK) != 0);
			this._error.txBufFullError = ((this._error.status & CE_TXFULL) != 0);
			this._error.parallelTmoutError = ((this._error.status & CE_PTO) != 0);
			this._error.deviceIOError = ((this._error.status & CE_IOE) != 0);
			this._error.notSelectedError = ((this._error.status & CE_DNS) != 0);
			this._error.outOfPaperError = ((this._error.status & CE_OOP) != 0);
			this._error.modeHandleError = ((this._error.status & CE_MODE) != 0);

			this._state.ctsHold = ((this.cs.bitfield & CTS_HOLD_BIT) != 0);
			this._state.dsrHold = ((this.cs.bitfield & DSR_HOLD_BIT) != 0);
			this._state.rlsdHold = ((this.cs.bitfield & RLSD_HOLD_BIT) != 0);
			this._state.xoffHold = ((this.cs.bitfield & XOFF_HOLD_BIT) != 0);
			this._state.xoffSent = ((this.cs.bitfield & XOFF_SENT_BIT) != 0);
			this._state.eof = ((this.cs.bitfield & EOF_BIT) != 0);
			this._state.trim = ((this.cs.bitfield & TRIM_BIT) != 0);
		...
		internal bool Reset()
		{
			...
			if (ClearCommError(_handle, out errors, IntPtr.Zero) == false)
			{
...
//				return false;
		...
		internal bool CtsHold
		{
			get { return this._state.ctsHold; }
		}
		...
		internal bool DsrHold
		{
			get { return this._state.dsrHold; }
		}
		...
		internal bool RlsdHold
		{
			get { return this._state.rlsdHold; }
		}
		...
		internal bool XoffHold
		{
			get { return this._state.xoffHold; }
		}
		...
		internal bool XoffSent
		{
			get { return this._state.xoffSent; }
		}
